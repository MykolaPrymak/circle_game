define(['eventemiter', 'log'], function(EventEmiter, Log) {
    AudioManager = EventEmiter.extend({
        init: function() {
            var self = this;
            ...
            function loadSoundFiles() {
                self.log.info('Loading sound files...');
                ...
                _.each(soundNames, function(name) {
                    self.loadSound(name, function(){
                        ...
                        if (!count) {
                            loadMusicFiles();
                            self.log.info('Sound loading complete');
                        }
                    })
                });
            };
            function loadMusicFiles() {
                self.log.info('Loading music files...');
                ...
                _.each(musicNames, function(name) {
                    self.loadMusic(name, function(){
                        ...
                        if (!count) {
                            ...
                            self.trigger('ready');
                            self.log.info('Music loading complete');
                        }
                    })
                });
            };
        ...
        toggle: function() {
          ...
          this.log.debug('Toogle audio: ', this.enabled);
        ...
        load: function(basePath, name, cb, chanels) {
            ...
            sound.addEventListener('canplaythrough', function() {
                self.log.info(name, 'is ready to play.');
            ...
            sound.addEventListener('error', function() {
                self.log.error('Error: ' + name + ' could not be loaded.');
